...
/* ------------------------- WHATSAPP CODE -------------------------- */
function kirimWA(id) {
var validated=!0;$("#"+id+" .thisRequired").each(function(){""!=$.trim($(this).val())&&"default"!=$.trim($(this).val())||$(this).addClass("focus")}),$("#"+id+" .thisRequired").each(function(){return""==$.trim($(this).val())?(validated=!1,$(this).parents("label").find(".validated").addClass("show"),$(this).focus(),!1):"default"==$.trim($(this).val())?(validated=!1,$(this).parents("label").find(".validated").addClass("show"),!1):void 0});
      if (validated === true) {
        var parameter = '';
        ...
        if (id == 'wa-checkout') {
          ...
          var nama_admin = namaAdmin,
              ...
              totalBerat = $('#'+id+' .productCart_totalberat').text(),
              satuanBerat = $('#'+id+' .productCart_satuanttlberat').text(),
              nama = $('#'+id+' .nama').val(),
              almtemail = $('#'+id+' .almtemail').val(),
          ...
          var parameter = url_wa + '?phone=' + kode_area + nomor_whatsapp + '&text=' +
              ...
              '*Total Berat :*%0A' + totalBerat + ' ' + satuanBerat + '%0A' +
              '==============================%0A%0A' +

              '*Nama :* ' + nama + '%0A' +
              '*Email :* ' + almtemail + '%0A' +
              '*Metode Pembayaran :* ' + pembayaran + '%0A%0A' +
		...
		var w = 960,h = 540,left = Number((screen.width / 2) - (w / 2)), tops = Number((screen.height / 2) - (h / 2)),popupWindow = window.open(this.href, '', 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=1, copyhistory=no, width=' + w + ', height=' + h + ', top=' + tops + ', left=' + left);popupWindow.focus();localStorage.removeItem('productCart_items');window.location.href = document.location.href;
return false;
      }
    }
